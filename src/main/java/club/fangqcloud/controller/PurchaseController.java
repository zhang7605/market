...
public class PurchaseController extends BaseController {
	...
	/*ajax方式按照查询条件分页查询进货信息*/
	@RequestMapping(value = { "/list" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void list(@ModelAttribute("productObj") Product productObj,String purchaseDate,@ModelAttribute("supplierObj") Supplier supplierObj,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (purchaseDate == null) purchaseDate = "";
		if(rows != 0)purchaseService.setRows(rows);
		List<Purchase> purchaseList = purchaseService.queryPurchase(productObj, purchaseDate, supplierObj, page);
	    /*计算总的页数和总的记录数*/
	    purchaseService.queryTotalPageAndRecordNumber(productObj, purchaseDate, supplierObj);
	    /*获取到总的页码数目*/
	...
	public String frontlist(@ModelAttribute("productObj") Product productObj,String purchaseDate,@ModelAttribute("supplierObj") Supplier supplierObj,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (purchaseDate == null) purchaseDate = "";
		List<Purchase> purchaseList = purchaseService.queryPurchase(productObj, purchaseDate, supplierObj, currentPage);
	    /*计算总的页数和总的记录数*/
	    purchaseService.queryTotalPageAndRecordNumber(productObj, purchaseDate, supplierObj);
	    /*获取到总的页码数目*/
	    int totalPage = purchaseService.getTotalPage();
	...
	public String delete(@PathVariable Integer purchaseId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  purchaseService.deletePurchase(purchaseId);
	            request.setAttribute("message", "进货删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "进货删除失败!");
				return "error";
	...
	public void OutToExcel(@ModelAttribute("productObj") Product productObj,String purchaseDate,@ModelAttribute("supplierObj") Supplier supplierObj, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(purchaseDate == null) purchaseDate = "";
        List<Purchase> purchaseList = purchaseService.queryPurchase(productObj,purchaseDate,supplierObj);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Purchase信息记录"; 
        String[] headers = { "进货id","进货商品","进货单价","进货数量","进货日期","供应商"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<purchaseList.size();i++) {
        	Purchase purchase = purchaseList.get(i); 
        	dataset.add(new String[]{purchase.getPurchaseId() + "",purchase.getProductObj().getProductName(),purchase.getPrice() + "",purchase.getPurchaseNum() + "",purchase.getPurchaseDate(),purchase.getSupplierObj().getSupplierName()});
        }
        /*
